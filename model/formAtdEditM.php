...
class formAtdEditM extends root
{
    ...
    public function updAtd($dados,$data){
        $con = $this->conectDB($data);
        $sql = "UPDATE atd SET nome_cliente = ?,telefone_cliente= ?, notas= ?, id_tecnologia= ?, id_bairro= ?, id_cidade= ?, id_status= ?,id_problema= ?,id_radios= ? WHERE id_atd = ?";
        $sqlpre = $con->prepare($sql);
        $sqlarray = array($dados['cliente'],$dados['telefone'],$dados['nota'],$dados['id_tec'],$dados['id_bairro'],$dados['id_cidade'],
            $dados['id_status'],$dados['id_problema'],$dados['id_radio'],$dados['id_atd']);
