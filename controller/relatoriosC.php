...
if ($_SESSION['level'] == 1) {
    header("location:index.php?pg=rel2");
}
...
class relatoriosC
{
    ...
    private function chamaTela()
    {
        ...
        //total de problemas
        $atdGeralProblemas = $rel->totalReg($cfg,$dataInicial,$dataFinal);
        $problemasAtdGerais = $rel->totalProblemas($cfg,$dataInicial,$dataFinal);
        $tableAtdProblemas = $this->dividirColunasProblemas($problemasAtdGerais);
        $prcAtdGeraisProblemas = $this->geraTabelaPorcetangemProblemas($problemasAtdGerais, $atdGeralProblemas);
    ...
    private function geraTabelaPorcetangemProblemas($dados, $total)
    {
        foreach ($dados as $chave => $valor) {
            $rs[$chave] = $this->calculoPorcentagem($total, $valor['cproblema']);
        }
        return $rs;
    }
    ...
    private function dividirColunasProblemas($problemaAtdGerais)
    {
        $counting = count($problemaAtdGerais);
        $dados;
        $cont = 0;
        
        for ($x = 0; $x < $counting; $x ++) {
            if (isset($problemaAtdGerais[($x + 1)]['cproblema'])) {
                $dados[$cont]['cproblema'] = $problemaAtdGerais[($x)]['cproblema'];
                $dados[$cont]['problema'] = $problemaAtdGerais[($x)]['problema'];
                $dados[$cont]['cproblema2'] = $problemaAtdGerais[($x + 1)]['cproblema'];
                $dados[$cont]['problema2'] = $problemaAtdGerais[($x + 1)]['problema'];
                unset($problemaAtdGerais[($x + 1)]);
            } else {
                $dados[$cont]['cproblema'] = $problemaAtdGerais[($x)]['cproblema'];
                $dados[$cont]['problema'] = $problemaAtdGerais[($x)]['problema'];
            }
